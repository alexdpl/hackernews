<!-- app/pages/admin/index.vue -->
<script setup lang="ts">
// Grazie agli auto-import nativi di Nuxt 3, useBlog e useAdminSettings sono già disponibili globalmente!
const { posts } = useBlog()
const { settings } = useAdminSettings()
</script>

<template>
  <div class="admin-container">
    <div class="admin-header">
      <span class="badge-tag">DKP Master Control Unit</span>
      <h1>Admin Command Center</h1>
      <p>Pannello di controllo unificato per la gestione totale dell'ecosistema DevKernelPulse.</p>
    </div>

    <!-- Metriche Rapide Globali -->
    <div class="metrics-grid">
      <div class="metric-card">
        <span class="metric-icon">📰</span>
        <div class="metric-info">
          <h3>{{ posts.length }}</h3>
          <p>Articoli Blog Pubblicati</p>
        </div>
      </div>
      <div class="metric-card">
        <span class="metric-icon">💼</span>
        <div class="metric-info">
          <h3>3</h3>
          <p>Posizioni Jobs Attive</p>
        </div>
      </div>
      <div class="metric-card">
        <span class="metric-icon">🛡️</span>
        <div class="metric-info">
          <h3>{{ settings.maintenanceMode ? 'Manutenzione' : 'Online / Sicuro' }}</h3>
          <p>Stato del Kernel</p>
        </div>
      </div>
    </div>

    <!-- Moduli di Navigazione Admin -->
    <div class="modules-grid">
      <NuxtLink to="/admin/blog" class="module-card">
        <div class="mod-icon">✍️</div>
        <h2>DKP Native Blog & News</h2>
        <p>Gestisci categorie, scrivi articoli con live preview e analizza i Karma Likes.</p>
        <span class="mod-link">Accedi al Modulo ↗</span>
      </NuxtLink>

      <NuxtLink to="/admin/jobs" class="module-card">
        <div class="mod-icon">💼</div>
        <h2>Jobs & Talent Acquisition</h2>
        <p>Pubblica nuove offerte di lavoro e gestisci le candidature tecniche.</p>
        <span class="mod-link">Accedi al Modulo ↗</span>
      </NuxtLink>

      <NuxtLink to="/admin/settings" class="module-card">
        <div class="mod-icon">⚙️</div>
        <h2>Global Configurations & Settings</h2>
        <p>Configura i feature flag, la modalità manutenzione e le chiavi pubbliche.</p>
        <span class="mod-link">Accedi al Modulo ↗</span>
      </NuxtLink>

      <div class="module-card disabled-card">
        <div class="mod-icon">🛒</div>
        <h2>DKP Ecosystem Shop (Prossimamente)</h2>
        <p>Pannello di controllo e-commerce nativo per merchandise e licenze enterprise.</p>
        <span class="mod-link">In Arrivo 🚀</span>
      </div>
    </div>
  </div>
</template>

<style scoped>
.admin-container { max-width: 1100px; margin: 2.5rem auto; padding: 0 1.5rem; }
.admin-header { margin-bottom: 2.5rem; text-align: center; }
.badge-tag { background: rgba(0, 220, 130, 0.15); color: #00a862; padding: 0.3rem 0.8rem; border-radius: 20px; font-size: 0.85rem; font-weight: 700; text-transform: uppercase; border: 1px solid rgba(0, 220, 130, 0.3); }
.admin-header h1 { font-size: 2.3rem; color: #020420; font-weight: 800; margin-top: 0.75rem; }
.admin-header p { color: #64748b; font-size: 1rem; }

/* Metrics */
.metrics-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1.5rem; margin-bottom: 3rem; }
.metric-card { background: #020420; border: 1px solid #1e293b; border-radius: 12px; padding: 1.5rem; display: flex; align-items: center; gap: 1.2rem; color: #ffffff; }
.metric-icon { font-size: 2.2rem; }
.metric-info h3 { font-size: 1.5rem; font-weight: 800; color: #00dc82; margin: 0; }
.metric-info p { color: #94a3b8; font-size: 0.85rem; margin: 0.2rem 0 0; }

/* Modules Grid */
.modules-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.5rem; }
.module-card { background: #020420; border: 1px solid #1e293b; border-radius: 12px; padding: 2rem; color: #ffffff; text-decoration: none; display: flex; flex-direction: column; transition: transform 0.2s, border-color 0.2s; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); }
.module-card:hover { transform: translateY(-4px); border-color: #00dc82; }
.disabled-card { opacity: 0.6; pointer-events: none; border-style: dashed; }
.mod-icon { font-size: 2.5rem; margin-bottom: 1rem; }
.module-card h2 { font-size: 1.25rem; font-weight: 700; color: #00dc82; margin-bottom: 0.5rem; }
.module-card p { color: #94a3b8; font-size: 0.9rem; line-height: 1.5; flex-grow: 1; margin-bottom: 1.5rem; }
.mod-link { color: #38bdf8; font-weight: 700; font-size: 0.9rem; }
</style>